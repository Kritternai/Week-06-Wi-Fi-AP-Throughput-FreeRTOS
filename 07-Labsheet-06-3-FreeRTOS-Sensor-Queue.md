...
> **ตอบ:** เพราะ FreeRTOS Queue มีกลไกจัดการ Critical Section อยู่เบื้องหลัง ทำให้มั่นใจได้ว่าขณะที่ Task หนึ่งกำลังเขียนข้อมูล อีก Task จะไม่สามารถเข้ามาอ่านข้อมูลที่ยังเขียนไม่เสร็จได้ (ป้องกัน Data Corruption และ Race Condition) ในขณะที่ตัวแปร Global ปกติไม่มีกลไกนี้ป้องกันครับ

2. ค่า **Stack High Water Mark** มีประโยชน์อย่างไรในการตรวจวินิจฉัยปัญหาบั๊กในระบบเรียลไทม์ (RTOS)?
> **ตอบ:** ค่านี้จะบอก "จุดที่เหลือพื้นที่ว่างน้อยที่สุด" ของ Stack ใน Task นั้นๆ ถ้าค่านี้น้อยจนเข้าใกล้ 0 แปลว่ามีความเสี่ยงที่จะเกิด Stack Overflow ซึ่งมักทำให้บอร์ดแฮงค์หรือรีสตาร์ท การดูค่า High Water Mark ทำให้เราปรับจูนขนาด Stack ตอนใช้ `xTaskCreate` ได้พอดี ไม่เปลือง RAM และระบบไม่แครชครับ
...
> **ตอบ:** ข้อมูลใน Queue จะสะสมจนเต็ม (Queue Full) พอคิวเต็มปุ๊บ `xQueueSend` จะไม่สามารถส่งข้อมูลชุดใหม่ลงไปได้ ทำให้ข้อมูลเซนเซอร์ใหม่ถูกทิ้ง (Data Loss) วิธีแก้คืออาจจะต้องทำ Data Aggregation (เฉลี่ยค่าก่อนส่ง) เพื่อลดความถี่การเขียนลงคิว หรือเพิ่มขนาดของ Queue ขึ้นไปอีกครับ
